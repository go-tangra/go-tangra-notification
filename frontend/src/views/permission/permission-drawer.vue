...
const data = ref<{
  resourceType: 'RESOURCE_TYPE_TEMPLATE' | 'RESOURCE_TYPE_CHANNEL';
  resourceId: string;
  resourceName?: string;
}>();
const loading = ref(false);
const permissions = ref<NotificationPermission[]>([]);
...
const grantForm = ref<{
  subjectType: 'SUBJECT_TYPE_USER' | 'SUBJECT_TYPE_ROLE' | 'SUBJECT_TYPE_CLIENT';
  subjectId: string;
...
const subjectOptions = computed(() => {
  ...
  } else if (grantForm.value.subjectType === 'SUBJECT_TYPE_ROLE') {
...
const isClientType = computed(() => grantForm.value.subjectType === 'SUBJECT_TYPE_CLIENT');
...
const title = computed(() => $t('notification.page.permission.title'));

const subjectTypeOptions = computed(() => [
  { value: 'SUBJECT_TYPE_USER', label: $t('notification.page.permission.user') },
  { value: 'SUBJECT_TYPE_ROLE', label: $t('notification.page.permission.role') },
  { value: 'SUBJECT_TYPE_CLIENT', label: $t('notification.page.permission.client') },
]);

const relationOptions = computed(() => [
  { value: 'RELATION_OWNER', label: $t('notification.page.permission.owner') },
  { value: 'RELATION_EDITOR', label: $t('notification.page.permission.editor') },
  { value: 'RELATION_VIEWER', label: $t('notification.page.permission.viewer') },
  { value: 'RELATION_SHARER', label: $t('notification.page.permission.sharer') },
]);
...
function formatDateTime(value: string | undefined) {
  if (!value) return $t('notification.page.permission.noExpiry');
  try {
...
async function loadPermissions() {
  ...
  try {
    ...
    permissions.value = (resp.permissions ?? []) as NotificationPermission[];
  } catch (e) {
...
async function handleGrant() {
  ...
  try {
    ...
    notification.success({
      message: $t('notification.page.permission.grantSuccess'),
    });
...
async function handleRevoke(permission: any) {
  ...
  Modal.confirm({
    title: $t('notification.page.permission.revokeAccess'),
    content: $t('notification.page.permission.revokeConfirm'),
    ...
    onOk: async () => {
      try {
        ...
        notification.success({
          message: $t('notification.page.permission.revokeSuccess'),
        });
        await loadPermissions();
      } catch (e) {
...
const columns: ColumnsType<any> = [
  ...
  {
    title: $t('notification.page.permission.subject'),
    dataIndex: 'subjectId',
    key: 'subjectId',
    width: 200,
    ellipsis: true,
  ...
  {
    title: $t('notification.page.permission.expiresAt'),
    dataIndex: 'expiresAt',
    key: 'expiresAt',
    width: 180,
...
const resourceTypeName = computed(() => {
  switch (data.value?.resourceType) {
    case 'RESOURCE_TYPE_TEMPLATE':
      return $t('notification.page.permission.template');
    case 'RESOURCE_TYPE_CHANNEL':
      return $t('notification.page.permission.channel');
    default:
      return '';
  }
});
  ...
  <Drawer :title="title" :footer="false" width="700px">
    <template v-if="data">
      <div class="mb-4 flex items-center justify-between">
        ...
        >
          {{ $t('notification.page.permission.grantAccess') }}
        </Button>
      </div>
      ...
      <div v-if="showGrantForm" class="bg-muted mb-4 rounded p-4">
        <Form layout="vertical" :model="grantForm" @finish="handleGrant">
          <div class="grid grid-cols-2 gap-4">
            <FormItem
              :label="$t('notification.page.permission.subjectType')"
              name="subjectType"
              :rules="[{ required: true }]"
            >
              <Select
            ...
            <FormItem
              v-if="!isClientType"
              :label="grantForm.subjectType === 'SUBJECT_TYPE_USER' ? $t('notification.page.permission.user') : $t('notification.page.permission.role')"
              name="subjectId"
              :rules="[{ required: true }]"
            >
              <Select
            ...
            <FormItem
              v-else
              :label="$t('notification.page.permission.clientId')"
              name="subjectId"
              :rules="[{ required: true }]"
            >
              <Input
                v-model:value="grantForm.subjectId"
                :placeholder="$t('notification.page.permission.clientIdPlaceholder')"
              />
            </FormItem>

            <FormItem
              :label="$t('notification.page.permission.relation')"
              name="relation"
              :rules="[{ required: true }]"
            >
              <Select
            ...
            <FormItem
              :label="$t('notification.page.permission.expiresAt')"
              name="expiresAt"
            >
              <DatePicker
                v-model:value="grantForm.expiresAt"
                show-time
                :placeholder="$t('notification.page.permission.noExpiry')"
                class="w-full"
              />
            </FormItem>
          </div>
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'action'">
              <Button
                ...
                :icon="h(LucideTrash)"
                :title="$t('notification.page.permission.revokeAccess')"
                @click="handleRevoke(record as NotificationPermission)"
              />
            </template>
          </template>
