...
// the schema-agnostic sqldump engine over notification's own tables (from ent
// migrate.Tables). Replaces the legacy per-field notification.service.v1.BackupService,
...
func NewSqlBackupService(ctx *bootstrap.Context) *SqlBackupService {
	...
	return &SqlBackupService{
		log:    ctx.NewLoggerHelper("notification/service/sql-backup"),
		engine: sqldump.New(dsn, sqldump.Options{Module: "notification", Tables: tables}),
	}
}
...
func (s *SqlBackupService) ImportBackup(stream commonV1.BackupService_ImportBackupServer) error {
	...
	if opts == nil {
		return errors.New("notification: first ImportBackup message must carry options")
	}
	...
	if err != nil {
		...
		return stream.SendAndClose(&commonV1.ImportBackupResponse{Success: false, Module: "notification", Warnings: []string{err.Error()}})
	}
	out := &commonV1.ImportBackupResponse{Success: true, Module: "notification", Warnings: res.Warnings}
