...
// AllChannelsSystem lists every channel of every tenant (KEK rotation).
func (s *Store) AllChannelsSystem(ctx context.Context) (out []Channel, err error) {
	err = s.Tx(ctx, Scope{System: true}, func(tx pgx.Tx) error {
		rows, err := tx.Query(ctx, "SELECT "+channelCols+" FROM channels c ORDER BY c.tenant_id, c.id")
		if err != nil {
			return err
		}
