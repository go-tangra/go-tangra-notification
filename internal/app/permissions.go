...
// permissions from the manifest; only notification knows the role grants.
func (a *App) SeedPermissions(ctx context.Context) error {
	...
	for _, p := range notificationmanifest.Permissions {
	...
	for _, slug := range []string{"owner", "admin", "member", "auditor", "operator"} {
		req.BuiltinGrants = append(req.BuiltinGrants, &authv1.BuiltinGrant{Role: slug, Permissions: notificationmanifest.Grants[slug]})
	}
