...
// TestRegister checks the request notification sends to auth (feature 019): module,
...
func TestRegister(t *testing.T) {
	...
	req := auth.got
	if auth.method != authv1.Authorization_RegisterPermissions_FullMethodName || req.GetModule() != "notification" || req.GetModuleDisplayName() != "Notifications" || !req.GetDeclaresRoles() {
		t.Fatalf("%s %v", auth.method, req)
	}
	if len(req.GetPermissions()) != len(notificationmanifest.Permissions) {
	...
	if len(roles) != 3 || len(roles["administrator"]) != len(notificationmanifest.Permissions)-1 || len(roles["sender"]) != 7 || len(roles["viewer"]) != 5 {
		t.Fatalf("%v", roles)
	}
	...
	for slug, refs := range notificationmanifest.Grants {
