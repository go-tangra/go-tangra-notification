// Package stream delivers live events to signed-in people: events are
// appended to one Valkey stream per tenant (fan-out across service
...
// Last-Event-ID (research R4). The same Valkey client counts rate limits.
package stream
...
func RateKey(kind, subject string, at time.Time) string {
	return "notif:rl:" + kind + ":" + subject + ":" + strconv.FormatInt(at.Unix()/60, 10)
}
