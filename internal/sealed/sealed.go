// Package sealed keeps channel provider settings encrypted at rest and out of
// every response: an envelope (per-row AES-256-GCM data key wrapped by the
// KEK, the scheme of the auth service) seals the JSON settings; reads replace
// credential fields with a marker; provider errors are scrubbed of stored
// credential values (research R1, SR-001).
package sealed
...
// Seal encrypts plaintext bound to associated data ("channel:<id>").
...
// AD is the associated data of a channel's settings.
func AD(channelID string) []byte { return []byte("channel:" + channelID) }
