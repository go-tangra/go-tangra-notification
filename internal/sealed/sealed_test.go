...
func TestSealOpenRoundTrip(t *testing.T) {
	...
	pt := []byte(`{"host":"smtp.example.org","password":"NOTIF-MARKER-PW-1"}`)
	blob, err := e.Seal(pt, AD("c1"))
	if err != nil {
		t.Fatal(err)
	}
	if bytes.Contains(blob, []byte("NOTIF-MARKER-PW-1")) {
		t.Fatal("plaintext in blob")
	}
	got, err := e.Open(blob, AD("c1"))
	if err != nil || !bytes.Equal(got, pt) {
		t.Fatalf("open: %v %q", err, got)
	}
	// Wrong associated data, tampering, wrong KEK and short blobs are refused.
	if _, err := e.Open(blob, AD("c2")); !errors.Is(err, ErrTampered) {
		t.Fatalf("ad mismatch: %v", err)
	}
	bad := append([]byte(nil), blob...)
	bad[len(bad)-1] ^= 1
	if _, err := e.Open(bad, AD("c1")); !errors.Is(err, ErrTampered) {
		t.Fatalf("tamper: %v", err)
	}
	e2, _ := NewEnvelope(bytes.Repeat([]byte{8}, 32))
	if _, err := e2.Open(blob, AD("c1")); !errors.Is(err, ErrTampered) {
		t.Fatalf("wrong kek: %v", err)
	}
	if _, err := e.Open(blob[:10], AD("c1")); !errors.Is(err, ErrTampered) {
		t.Fatalf("short: %v", err)
	}
	if _, err := e.Seal(bytes.Repeat([]byte("x"), MaxSettingsBytes+1), AD("c1")); !errors.Is(err, ErrTooLarge) {
...
func TestRedactMergeScrub(t *testing.T) {
	...
	stored := Settings{"host": "h", "port": float64(25), "password": "NOTIF-MARKER-PW-2"}
	r := Redact(stored, fields)
	if r["password"] != Marker || r["host"] != "h" {
		t.Fatalf("redact %v", r)
	}
	if v, ok := Redact(Settings{"password": ""}, fields)["password"]; ok {
		t.Fatalf("empty credential should vanish: %v", v)
	}
	if _, ok := Public(stored, fields)["password"]; ok {
		t.Fatal("public keeps password")
	}
	// Merge: marker/omitted keep, "" clears, value replaces.
	if m := Merge(stored, Settings{"host": "h2", "password": Marker}, fields); m["password"] != "NOTIF-MARKER-PW-2" || m["host"] != "h2" {
		t.Fatalf("merge marker %v", m)
	}
	if m := Merge(stored, Settings{"host": "h2"}, fields); m["password"] != "NOTIF-MARKER-PW-2" {
		t.Fatalf("merge omitted %v", m)
	}
	if m := Merge(stored, Settings{"host": "h2", "password": ""}, fields); m["password"] != nil {
		t.Fatalf("merge clear %v", m)
	}
	if m := Merge(stored, Settings{"password": "new"}, fields); m["password"] != "new" {
		t.Fatalf("merge replace %v", m)
	}
	if m := Merge(Settings{}, Settings{"password": Marker}, fields); m["password"] != nil {
		t.Fatalf("merge marker without stored %v", m)
	}
	// Scrub removes the credential (raw and base64) and later lines.
	enc := base64.StdEncoding.EncodeToString([]byte("NOTIF-MARKER-PW-2"))
	s := Scrub("535 auth failed for NOTIF-MARKER-PW-2 ("+enc+")\nsecond line NOTIF-MARKER-PW-2", stored, fields)
	if strings.Contains(s, "NOTIF-MARKER") || strings.Contains(s, enc) || strings.Contains(s, "second") {
		t.Fatalf("scrub %q", s)
	}
...
func TestLoadKEK(t *testing.T) {
	...
	t.Setenv("NOTIF_KEK", base64.RawStdEncoding.EncodeToString(raw))
	if k, err := LoadKEK("env", "", "NOTIF_KEK"); err != nil || !bytes.Equal(k, raw) {
		t.Fatalf("env: %v", err)
	}
	t.Setenv("NOTIF_KEK", "")
	if _, err := LoadKEK("env", "", "NOTIF_KEK"); err == nil {
...
func TestSealRandomFailure(t *testing.T) {
	...
	for n := 0; n < 3; n++ {
		...
		if _, err := e.Seal([]byte("{}"), AD("c")); err == nil {
