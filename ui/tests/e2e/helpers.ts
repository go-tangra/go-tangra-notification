...
export async function signIn(page: Page, email: string, password: string): Promise<void> {
  ...
  if (await page.getByTestId('mfa-enrol').isVisible()) {
    ...
    let period = Math.floor(Date.now() / 30000)
    while (period === lastPeriod()) {
      await page.waitForTimeout(1000)
      period = Math.floor(Date.now() / 30000)
    }
    rememberPeriod(period)
    await page.getByTestId('mfa').locator('input').fill(totp(storedSecret()))
...
/** Opens a module route through the shell and waits for its remote to mount. */
export async function goto(page: Page, path: string, ready: string): Promise<void> {
