...
COPY services/notification/ui/ ./services/notification/ui/
RUN npm run -w services/notification/ui build

FROM golang:1.26-alpine AS build
RUN apk add --no-cache git ca-certificates
WORKDIR /src
COPY . .
COPY --from=ui /w/services/notification/ui/dist ./services/notification/ui/dist
